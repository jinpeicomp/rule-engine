...
public class DynamicRule {
    ...
    static {
        try (InputStream is = DynamicRule.class.getResourceAsStream("/com/jinpei/re/dynamic/template.drl");
    ...
    public static DynamicRule build(Activity activity) {
        if (StringUtils.isBlank(RULE_TEMPLATE)) {
            return null;
        }

        String fileName = String.format("src/main/resources/dynamic/rules/%d.drl", activity.getId());
        ...
        variables.put("region", activity.getRegion());
        variables.put("terminalType", activity.getTerminalType());
        variables.put("loginType", activity.getLoginType());
